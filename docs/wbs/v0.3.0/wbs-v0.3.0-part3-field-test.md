...
| 7.8 | Cost measurement — LLM cost per candidate, per promoted rule | [#653](https://github.com/deghosal-2026/CauterRule/issues/653) | tooling done (`measure_cost.py` + `cost.py` + 1k corpus); table pending sweep |
...
| 7.10 | Cross-session repeat-failure reduction measurement — before/after protocol | [#663](https://github.com/deghosal-2026/CauterRule/issues/663) | tooling done (`cross_session.py` + runner `--cross-session`); protocol run pending |
