...
def test_candidate_validation() -> None:
    ...
    with pytest.raises(ValueError, match="extraction_pass"):
        CandidateRule(when=RuleWhen(trigger="t"), do=RuleDo(directive="d"), confidence=0.5, extraction_pass=0)
    with pytest.raises(ValueError, match="reasoning"):
        CandidateRule(when=RuleWhen(trigger="t"), do=RuleDo(directive="d"), confidence=0.5, reasoning="   ")
    with pytest.raises(ValueError, match="template"):
        CandidateRule(when=RuleWhen(trigger="t"), do=RuleDo(directive="d"), confidence=0.5, template="   ")
