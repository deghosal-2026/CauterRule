...
def test_trajectory_path_failure(tmp_path: Path) -> None:
    t = Trajectory(id="T-001", timestamp="2026-09-03T18:25:00Z", task="task", steps=(), success=False)
...
def test_trajectory_path_success() -> None:
    t = Trajectory(id="T-002", timestamp="2026-09-03T18:25:00Z", task="task", steps=(), success=True)
    p = trajectory_path(t)
    assert p.name == "success-T-002.jsonl"


def test_trajectory_path_sanitize() -> None:
    t = Trajectory(id="T/001:bad", timestamp="2026-09-03T18:25:00Z", task="task", steps=(), success=False)
    p = trajectory_path(t)
