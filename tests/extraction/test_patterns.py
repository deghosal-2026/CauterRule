...
def _traj(id: str, failure_class: str) -> Trajectory:
    return Trajectory(id=id, timestamp="t", task="task", steps=(), success=False, failure_class=failure_class)


def test_detect_repeated() -> None:
    trajs = [_traj(f"T-{i}", "git/push") for i in range(5)] + [_traj(f"T-{i}", "python/import") for i in range(2)]
...
def test_detect_sorted() -> None:
    trajs = [_traj("T-1", "a"), _traj("T-2", "a"), _traj("T-3", "a"), _traj("T-4", "b"), _traj("T-5", "b"), _traj("T-6", "b"), _traj("T-7", "b")]
...
def test_extract_stronger_rule() -> None:
    ...
    assert "3" in rule.reasoning  # type: ignore
