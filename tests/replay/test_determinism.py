...
def test_deterministic_same_result() -> None:
    cand = _cand("git push")
    trajs = [
        Trajectory(id="T-1", timestamp="t", task="git push fails", steps=(Step(1, "bash", error="err"),), success=False),
        Trajectory(id="T-2", timestamp="t", task="docker fails", steps=(Step(1, "bash", error="err"),), success=False),
    ]
...
def test_deterministic_ordering() -> None:
    cand = _cand("git push")
    trajs = [
        Trajectory(id="T-2", timestamp="t", task="git push", steps=(Step(1, "bash", error="err"),), success=False),
        Trajectory(id="T-1", timestamp="t", task="git push", steps=(Step(1, "bash", error="err"),), success=False),
    ]
...
def test_corpus_change_invalidates_cached_verdict() -> None:
    ...
    edited = [
        Trajectory(id="T-1", timestamp="t", task="git push", steps=(Step(1, "bash", error="changed error text"),), success=False),
    ]
    r1 = deterministic_replay(cand, trajs)
...
def test_same_corpus_same_hash() -> None:
    ...
    trajs = [
        Trajectory(id="T-1", timestamp="t", task="git push", steps=(Step(1, "bash", error="err"),), success=False),
        Trajectory(id="T-2", timestamp="t", task="docker", steps=(Step(1, "bash", error="err"),), success=False),
    ]
    r1 = deterministic_replay(cand, trajs)
    r2 = deterministic_replay(cand, trajs)
    assert r1.corpus_hash == r2.corpus_hash
