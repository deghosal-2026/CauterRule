...
def _traj(
    task: str = "git push", error: str = "non-fast-forward", success: bool = False
) -> Trajectory:
    return Trajectory(
...
def test_qwen_alias_expansion() -> None:
    ...
    traj = Trajectory(
        ...
        timestamp="t",
        task="git push fails",
        steps=(Step(1, "bash", error="exit code 128"),),
        success=False,
...
def test_short_generic_trigger_accepted_with_context() -> None:
    ...
    traj = Trajectory(
        id="T-ctx",
        timestamp="t",
        task="something fatal",
        steps=(Step(1, "bash", error="error occurred"),),
        success=False,
    )
    assert rule_matches(cand, traj)
...
def test_noisy_haystack_penalizes_score() -> None:
    ...
    small_traj = Trajectory(
        ...
        steps=(Step(1, "bash", error="failure"),),
        success=False,
    )
    big_traj = Trajectory(
        id="T-lg",
        ...
        steps=(
            Step(1, "bash", error="failure pushing to registry after retry with fallback snapshot"),
        ),
        success=False,
    )
...
def test_rule_matches_rejects_cross_domain_match() -> None:
    ...
    traj = Trajectory(
        id="T-cross",
        timestamp="t",
        task="docker build failed on push",
        steps=(Step(1, "bash", error="rejected"),),
        success=False,
...
def test_rule_matches_allows_same_domain_match() -> None:
    ...
    traj = Trajectory(
        id="T-same",
        timestamp="t",
        task="docker build failed",
        steps=(Step(1, "bash", error="exit 1"),),
        success=False,
        failure_class="docker/build",
    )
    assert rule_matches(cand, traj)
...
def test_check_domain_mismatch_is_invoked(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def spy(candidate: CandidateRule, trajectory: Trajectory) -> bool:
        calls["n"] += 1
        return bool(real(candidate, trajectory))
...
def test_domain_mismatch_treats_related_domains_as_compatible() -> None:
    ...
    pip_traj = Trajectory(
        id="T-pip",
        timestamp="t",
        task="pip install fails",
        steps=(Step(1, "bash", error="dependency conflict"),),
        success=False,
    ...
    k8s_traj = Trajectory(
        ...
        timestamp="t",
        task="kubectl apply",
        steps=(Step(1, "bash", error="no matches for kind"),),
        success=False,
    ...
    ci_traj = Trajectory(
        ...
        steps=(Step(1, "bash", error="timeout"),),
        success=False,
