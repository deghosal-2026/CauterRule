...
GOLDEN_SCENARIOS = [
    ...
    },
    {
        ...
        "steps": [
            Step(1, "bash", "python -c 'import requests'", None, "ModuleNotFoundError: No module named 'requests'"),
    ...
    },
    {
        ...
        "steps": [
            ...
            Step(2, "edit", "RUN apt-get update && apt-get install -y libpq-dev", "Dockerfile updated"),
        ],
    },
    {
        ...
        "steps": [
            Step(1, "bash", "pip install flask>=2.0", None, "ERROR: pip's dependency resolver conflict"),
            Step(2, "bash", "python -m venv .venv && source .venv/bin/activate && pip install flask", "Installed flask 2.3"),
        ],
    },
    {
        ...
        "steps": [
            Step(1, "bash", "kubectl apply -f my-custom-resource.yaml", None, "error: unable to recognize: no matches for kind"),
            Step(2, "bash", "kubectl apply -f crd.yaml && kubectl apply -f my-custom-resource.yaml", "customresource created"),
        ],
    },
    {
        ...
        "steps": [
            Step(1, "bash", "pytest tests/", None, "FAILED test_example.py::test_transform - AssertionError"),
            Step(2, "bash", "pytest tests/ -s --tb=long", "test_transform FAILED, stdout: actual=42 expected=100"),
        ],
    },
    {
        ...
        "steps": [
            Step(1, "web_fetch", "GET https://api.example.org/v1/data", None, "429 Too Many Requests"),
    ...
    },
    {
        ...
        "steps": [
            Step(1, "bash", "terraform apply -auto-approve", None, "Error acquiring state lock: ConditionalCheckFailedException"),
    ...
    },
    {
        ...
        "steps": [
            ...
            Step(2, "search", "WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, 'submit-btn')))", "element found"),
        ],
    },
    {
...
for i in range(1, 21):
    ...
    COUNTEREXAMPLES.append(Trajectory(
        id=f"counterex-{i:03d}",
        timestamp=TS,
        task=f"{counterpart} (expected failure that succeeds)",
        steps=(
            Step(1, "bash" if i % 2 == 0 else "web_fetch", f"step_1_input_{i}", f"step_1_output_{i}"),
            Step(2, "bash" if i % 2 == 0 else "edit", f"step_2_input_{i}", f"step_2_output_{i}"),
        ),
        success=True,
        quality_label="clear",
        domain=domain,
...
for i in range(31, 51):
    success = i % 3 != 0
    domain = ["coding", "devops", "research", "support", "browser_automation"][i % 5]
    SYNTHETIC.append(Trajectory(
        id=f"synth-{i:03d}",
        timestamp=TS,
        task=f"Synthetic task {i} — {'success' if success else 'failure'}",
        steps=(
            Step(1, "bash", f"input_{i}_1", f"output_{i}_1" if success else None, None if success else f"error_{i}_1"),
