...
SUCCESS_SCENARIOS = [
    ...
    (
        "pytest-pass",
        "Run Python tests that pass",
        "python",
        "bash",
        [("pytest tests/ -v", "42 passed in 3.2s")],
    ),
    ("flake8-clean", "Run flake8 linting", "python", "bash", [("flake8 src/", "")]),
    (
        "docker-ps",
    ...
    ("ci-lint", "CI linting stage passes", "ci", "bash", [("make lint", "All lint checks passed")]),
    ("ci-build", "CI build stage completes", "ci", "bash", [("make build", "Build successful")]),
    ("ci-test", "CI test stage passes", "ci", "bash", [("make test", "All tests passed")]),
    (
        "ci-deploy",
        "CI deploy stage succeeds",
        "ci",
        "bash",
    ...
    ("shell-mkdir", "Create a directory", "shell", "bash", [("mkdir -p /tmp/new-project/src", "")]),
    ("shell-cp", "Copy files", "shell", "bash", [("cp -r src/ /tmp/backup/", "")]),
    (
        "shell-echo",
    ...
    (
        ...
        [
            (
                "requests.get('https://api.publicapis.org/entries')",
                '{"count": 1425, "entries": [...]}',
            )
        ],
    ),
    ...
    (
        ...
        [
            (
                ...
                "2026-09-07 12:00:00 URL:... Saved [123456]",
            )
        ],
    ),
    (
...
NEGATIVE_SCENARIOS = [
    ...
    (
        "user-opinion",
        "User expresses opinion about code style",
        "coding",
    ...
    (
        "browser-page-load",
        "Browser loads a page successfully",
        "browser_automation",
    ...
    (
        "browser-wait",
        "Browser waits for element",
        "browser_automation",
        [
            (
                "search",
                "WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, 'done')))",
                "<Element 'div' id='done'>",
            )
        ],
...
NEARMISS_SCENARIOS = [
    ...
    (
        ...
        "git/push/temporary",
        ["git", "push", "retry", "nearmiss"],
    ),
    (
        "docker-build-retry",
        "Docker build fails, retry succeeds",
        "docker",
        [
        ...
        ["docker", "build", "retry", "nearmiss"],
    ),
    (
        "pip-install-retry",
        "pip install fails, retry works",
        "python",
        [
            (
                "bash",
                "pip install requests",
                "ERROR: Could not install packages due to OSError: [Errno 28] No space left on device",
            ),
            ("bash", "pip install requests --no-cache-dir", "Successfully installed requests"),
        ],
        "python/pip/disk-full",
        ["python", "pip", "retry", "nearmiss"],
    ),
    (
        "test-retry",
        "Test fails, rerun passes",
        "python",
        [
            (
                "bash",
                "pytest tests/test_flaky.py",
                "FAILED tests/test_flaky.py::test_something - AssertionError",
            ),
            ("bash", "pytest tests/test_flaky.py --flaky-reruns=1", "PASSED (1 rerun)"),
        ],
        "test/flaky/retry",
        ["test", "flaky", "retry", "nearmiss"],
    ),
    (
        "npm-install-retry",
        "npm install fails, retry succeeds",
        "javascript",
        [
        ...
        "npm/install/network",
        ["npm", "install", "retry", "nearmiss"],
    ),
    (
        "kubectl-apply-retry",
        "kubectl apply fails, retry succeeds",
        "devops",
        [
            (
                "bash",
        ...
        "k8s/apply/temporary",
        ["kubectl", "apply", "retry", "nearmiss"],
    ),
    (
        "terraform-plan-retry",
        "terraform plan fails, retry succeeds",
        "devops",
        [
        ...
        ["terraform", "lock", "retry", "nearmiss"],
    ),
    (
        "api-timeout-retry",
        "API call times out, retry succeeds",
        "research",
        [
            ("web_fetch", "GET https://api.example.com/data", "504 Gateway Timeout"),
            (
                "web_fetch",
                "GET https://api.example.com/data (retry after 1s)",
                '{"data": [1, 2, 3]}',
            ),
        ],
        "api/timeout/transient",
        ["api", "timeout", "retry", "nearmiss"],
    ),
    (
        "browser-stale-retry",
        "Browser stale element, re-find succeeds",
        "browser_automation",
        [
        ...
        ["browser", "selenium", "stale", "nearmiss"],
    ),
    (
        "deploy-timeout-retry",
        "Deploy times out, retry succeeds",
        "devops",
        [
            (
                "bash",
                "kubectl rollout status deployment/myapp",
                "error: deployment roll out timed out",
            ),
            (
                "bash",
                "kubectl rollout restart deployment/myapp && kubectl rollout status deployment/myapp",
                'deployment "myapp" successfully rolled out',
            ),
        ],
        "ci/deploy/timeout",
        ["deploy", "timeout", "retry", "nearmiss"],
    ),
    (
        "git-merge-retry",
        "Git merge conflict, resolved and merged",
        "git",
        [
            ("git", "git merge feature/login", "Auto-merging src/auth.py\nCONFLICT in src/auth.py"),
            (
                "git",
                "git add src/auth.py && git commit -m 'resolve merge conflict'",
                "[main abc123] resolve merge conflict",
            ),
        ],
        "git/merge/conflict",
        ["git", "merge", "conflict", "nearmiss"],
    ),
    (
        "docker-compose-retry",
        "Docker compose fails, retry succeeds",
        "docker",
        [
        ...
        "docker/compose/port",
        ["docker", "compose", "port", "nearmiss"],
    ),
    (
        "pip-conflict-retry",
        "pip dependency conflict, resolved with workaround",
        "python",
        [
        ...
        "python/pip/conflict",
        ["python", "pip", "conflict", "nearmiss"],
    ),
    (
        "ssh-connect-retry",
        "SSH connection fails, retry succeeds",
        "devops",
        [
        ...
        "ssh/connect/refused",
        ["ssh", "connect", "retry", "nearmiss"],
    ),
    (
        "database-connect-retry",
        "Database connection fails, retry succeeds",
        "devops",
        [
            ("bash", "psql -h localhost -U user mydb", "psql: error: connection refused"),
            (
                "bash",
                "sleep 3 && psql -h localhost -U user mydb",
                "psql (16.0)\nType 'help' for help.",
            ),
        ],
        "database/connect/refused",
        ["database", "connect", "retry", "nearmiss"],
    ),
    (
        "certificate-retry",
        "SSL cert error, retry works",
        "research",
        [
            (
                "web_fetch",
        ...
        ["ssl", "certificate", "verify", "nearmiss"],
    ),
    (
        "rate-limit-retry",
        "API rate limited, retry after wait",
        "research",
        [
        ...
        ["api", "rate-limit", "retry", "nearmiss"],
    ),
    (
        "docker-pull-retry",
        "Docker pull fails, retry succeeds",
        "docker",
        [
            (
                "bash",
        ...
        "docker/pull/auth",
        ["docker", "pull", "auth", "nearmiss"],
    ),
    (
        "browser-navigate-retry",
        "Browser navigation times out, retry succeeds",
        "browser_automation",
        [
            (
                "search",
                "driver.get('https://slow.example.com')",
                "TimeoutException: page load timed out",
            ),
            (
                "search",
                "driver.set_page_load_timeout(60); driver.get('https://slow.example.com')",
                "Page loaded",
            ),
        ],
        "browser/navigation/timeout",
        ["browser", "navigation", "timeout", "nearmiss"],
    ),
    (
        "pytest-assertion-retry",
        "Test assertion fails, fix and retry",
        "python",
        [
            (
                "bash",
                "pytest tests/",
                "FAILED test_example.py::test_compare - AssertionError: assert 42 == 100",
            ),
            ("bash", "pytest tests/", "PASSED (1 passed)"),
        ],
        "python/test/assertion",
        ["test", "assertion", "retry", "nearmiss"],
    ),
    (
        "ci-cache-miss",
        "CI cache miss, rebuild succeeds",
        "ci",
        [
        ...
        "ci/cache/miss",
        ["ci", "cache", "miss", "nearmiss"],
    ),
    (
        "ci-timeout",
        "CI job times out, rerun works",
        "ci",
        [
        ...
        "ci/test/timeout",
        ["ci", "test", "timeout", "nearmiss"],
    ),
    (
        "browser-alert",
        "Browser alert appears, handled",
        "browser_automation",
        [
            (
                "search",
                "element.click()",
                "UnexpectedAlertPresentException: Alert text: Are you sure?",
            ),
            ("search", "Alert(driver).accept(); element.click()", ""),
        ],
        "browser/alert/unexpected",
        ["browser", "alert", "nearmiss"],
    ),
    (
        "docker-oom",
        "Container OOM, restarts with more memory",
        "docker",
        [
        ...
        "docker/oom/kill",
        ["docker", "oom", "memory", "nearmiss"],
    ),
    (
        "kubectl-rollback",
        "Deploy fails, rollback succeeds",
        "devops",
        [
        ...
        "k8s/deploy/rollback",
        ["kubectl", "deploy", "rollback", "nearmiss"],
    ),
    (
        "terraform-apply-retry",
        "Terraform apply fails, retry succeeds",
        "devops",
        [
        ...
        ["terraform", "apply", "conflict", "nearmiss"],
    ),
    (
        "shell-command-retry",
        "Shell command fails, retry succeeds",
        "shell",
        [
        ...
        ["shell", "permission", "retry", "nearmiss"],
    ),
    (
        "pip-install-version",
        "pip install wrong version, correct version works",
        "python",
        [
        ...
        "python/pip/version",
        ["python", "pip", "version", "nearmiss"],
    ),
    (
        "npm-build-fail",
        "npm build fails, retry works",
        "javascript",
        [
        ...
        "npm/build/fail",
        ["npm", "build", "cache", "nearmiss"],
    ),
    (
        "git-commit-hook",
        "Git commit hook fails, bypass works",
        "git",
        [
        ...
        "git/commit/hook",
        ["git", "commit", "hook", "nearmiss"],
    ),
    (
        "browser-frame",
        "Browser frame switch fails, retry works",
        "browser_automation",
        [
            ("search", "driver.switch_to.frame('nonexistent')", "NoSuchFrameException"),
            (
                "search",
                "driver.switch_to.default_content(); driver.switch_to.frame('main-frame')",
                "",
            ),
        ],
        "browser/frame/switch",
        ["browser", "frame", "switch", "nearmiss"],
    ),
    (
        "docker-volume",
        "Docker volume mount fails, retry works",
        "docker",
        [
            (
                "bash",
        ...
        "docker/volume/mount",
        ["docker", "volume", "mount", "nearmiss"],
    ),
    (
        "api-500-retry",
        "API returns 500, retry succeeds",
        "research",
        [
        ...
        "api/server/error",
        ["api", "500", "retry", "nearmiss"],
    ),
    (
        "dns-resolution-retry",
        "DNS resolution fails, retry succeeds",
        "support",
        [
        ...
        ["dns", "resolution", "retry", "nearmiss"],
    ),
    (
        "network-flaky",
        "Network flaky, retry succeeds",
        "support",
        [
            ("bash", "curl -s https://api.example.com", "curl: (7) Failed to connect"),
            ("bash", "curl -s --retry 3 https://api.example.com", '{"status": "ok"}'),
        ],
        "network/flaky/connect",
        ["network", "flaky", "retry", "nearmiss"],
    ),
    (
        "support-connection",
        "Connection to support portal fails, retry succeeds",
        "support",
        [
            ("bash", "curl https://support.example.com/ticket", "curl: (28) Connection timed out"),
            (
                "bash",
                "curl https://support.example.com/ticket --connect-timeout 60",
                '{"ticket": 12345}',
            ),
        ],
        "support/connection/timeout",
        ["support", "connection", "timeout", "nearmiss"],
    ),
]
...
def main() -> int:
    ...
    print("\nNormalizing...")
    for path in [
        "field-test/corpus/curated/successes",
        "field-test/corpus/curated/failures/negative",
        "field-test/corpus/curated/nearmiss",
    ]:
        r = subprocess.run(
            [sys.executable, "scripts/normalize-corpus.py", "--path", path],
            capture_output=True,
            text=True,
        )
