...
def gen_lifecycle() -> int:
    stages = [
        ...
        (
            "outcome-evolving",
            "should_extract",
            "Rule outcome shifted from prevented to neutral over sessions",
        ),
    ]
    recs: list[dict] = []
        ...
        for i in range(1, 11):
            ...
            recs.append(
                _traj(
                    ...
                    [
                        _step(
                            1, "bash", f"run scenario {i}", None if success else f"{stage} failure"
                        )
                    ],
                    success=success,
...
def gen_mcp() -> int:
    ...
    for i in range(1, 6):
        recs.append(
            _traj(
                f"mcp-valid-{i:03d}",
                f"MCP valid report_failure {i}",
                [_step(1, "mcp", "report_failure", None)],
                success=False,
                failure_point="step_1",
                failure_class="mcp/valid",
                domain="ci",
                severity="low",
                quality_label="clear",
                tags=("mcp", "valid"),
    ...
    for missing in ["trajectory", "error", "steps", "success", "task"]:
        for j in (1, 2):
            recs.append(
                _traj(
                    f"mcp-malformed-{missing}-{j:03d}",
                    f"MCP malformed report_failure missing {missing} ({j})",
                    [_step(1, "mcp", "report_failure", f"schema validation: missing {missing}")],
                    success=False,
                    failure_point="step_1",
                    failure_class="mcp/malformed",
                    domain="ci",
                    severity="medium",
                    quality_label="misleading",
                    tags=("mcp", "malformed", missing),
    ...
    for i in range(1, 6):
        recs.append(
            _traj(
                f"mcp-abusive-{i:03d}",
                f"MCP abusive burst {i}",
                [_step(1, "mcp", "report_failure", "rate limit exceeded")],
                success=False,
                failure_point="step_1",
                failure_class="mcp/rate-limit",
                domain="ci",
                severity="medium",
                quality_label="misleading",
                tags=("mcp", "abusive", "rate-limit"),
                expected_outcome="should_reject",
    ...
    for ev in events:
        for i in range(1, 6):
            ...
            recs.append(
                _traj(
                    f"otel-{ev}-{i:03d}",
                    f"OTEL span emission on {ev} event {i}",
...
def gen_cost() -> int:
        ...
        *,
        ...
        recs.append(
            _traj(
                ...
                domain=domain,
                severity=severity if not success else None,
                quality_label=quality,
                tags=("cost", kind, domain),
                expected_outcome=outcome,
                rationale=rationale,
    ...
    for i in range(300):
        add(
            "failure-positive",
            success=False,
            outcome="should_extract",
            domain=domains[i % 8],
            tool=tools[i % 8],
            fc=f"{domains[i % 8]}/failure",
            err=f"{domains[i % 8]} failure {i}",
    ...
    for i in range(200):
        add(
            ...
            tool=tools[i % 8],
            fc="cosmetic/warning",
            err="warning only, not actionable",
    ...
    for i in range(200):
        ...
        add(
            "raw-mixed",
            success=(outcome == "should_silence" and i % 2 == 0),
            outcome=outcome,
            domain=domains[i % 8],
            tool=tools[i % 8],
            fc="raw/mixed",
            err=None if outcome == "should_silence" else f"raw mixed output {i}",
