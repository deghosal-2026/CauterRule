...
def build_counterexample_corpus() -> list[Trajectory]:
    ...
    t1 = Trajectory(
        ...
        steps=(
            Step(step_number=1, tool="bash", input="git push origin feature/login", output="Everything up-to-date", error=None, state=None),
            Step(step_number=2, tool="bash", input="git push --force origin feature/login", output="remote: accepted\nTo origin", error=None, state=None),
        ),
        success=True,
        quality_label="clear",
        domain="coding",
    ...
    t2 = Trajectory(
        ...
        steps=(
            Step(step_number=1, tool="bash", input="kubectl describe quota", output="ResourceQuota: 10 pods used / 10 limit → adjusted to 5", error=None, state={"namespace": "dev"}),
            Step(step_number=2, tool="bash", input="kubectl apply -f reduced-deploy.yaml", output="deployment.apps/svc created", error=None, state=None),
        ),
        success=True,
        quality_label="clear",
        domain="devops",
    ...
    t3 = Trajectory(
        ...
        steps=(
            Step(step_number=1, tool="web_fetch", input="https://openaccess.example.org/paper", output="Full text retrieved", error=None, state={"method": "oa"}),
            Step(step_number=2, tool="read", input="paper content", output="Extracted 3 sections", error=None, state=None),
        ),
        success=True,
        quality_label="clear",
        domain="research",
