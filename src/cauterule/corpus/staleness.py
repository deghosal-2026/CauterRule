...
def build_staleness_corpus() -> list[Trajectory]:
    ...
    t1 = Trajectory(
        ...
        steps=(
            Step(step_number=1, tool="read", input="def old_func(): pass", output="legacy code", error=None, state=None),
            Step(step_number=2, tool="edit", input="refactor to modern syntax", output="def new_func(): ...", error=None, state=None),
        ),
        success=True,
        quality_label="clear",
        domain="coding",
    ...
    t2 = Trajectory(
        ...
        steps=(
            Step(step_number=1, tool="bash", input="docker-compose up -d", output="Starting services...", error=None, state=None),
            Step(step_number=2, tool="bash", input="docker-compose ps", output="all services running", error=None, state=None),
        ),
        success=True,
        quality_label="clear",
        domain="devops",
    ...
    t3 = Trajectory(
        ...
        steps=(
            Step(step_number=1, tool="bash", input="npm test", output="test suite failed", error="segfault", state=None),
            Step(step_number=2, tool="bash", input="npm run build", output="build failed", error="out of memory", state=None),
        ),
        success=False,
