...
def build_nearmiss_corpus() -> list[Trajectory]:
    ...
    t1 = Trajectory(
        ...
        steps=(
            Step(step_number=1, tool="bash", input="git push origin main", output="! [rejected] non-fast-forward", error="failed to push", state=None),
            Step(step_number=2, tool="bash", input="git pull --rebase && git push origin main", output="Everything up-to-date\nTo origin\n* main", error=None, state=None),
        ),
        success=True,
        failure_point=None,
        failure_class=None,
    ...
    t2 = Trajectory(
        ...
        steps=(
            Step(step_number=1, tool="bash", input="kubectl apply -f deploy.yaml", output="pod/svc-1 created", error=None, state={"pod": "svc-1"}),
            Step(step_number=2, tool="bash", input="kubectl get pods svc-1", output="OOMKilled", error="OOMKilled", state=None),
            Step(step_number=3, tool="edit", input="add memory limits to deploy.yaml", output="deploy.yaml updated", error=None, state=None),
            Step(step_number=4, tool="bash", input="kubectl apply -f deploy.yaml", output="pod/svc-2 created\nRunning", error=None, state=None),
        ),
        success=True,
    ...
    t3 = Trajectory(
        ...
        steps=(
            Step(step_number=1, tool="web_fetch", input="https://api.example.org/data", output=None, error="429 Too Many Requests", state={"retry_after": "5"}),
            Step(step_number=2, tool="bash", input="sleep 5", output="", error=None, state=None),
            Step(step_number=3, tool="web_fetch", input="https://api.example.org/data", output="[{'id': 1, 'name': 'test'}]", error=None, state=None),
        ),
        success=True,
        quality_label="clear",
        domain="research",
