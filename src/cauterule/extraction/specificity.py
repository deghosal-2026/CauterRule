...
_CONCRETE_MARKERS: frozenset[str] = frozenset(
    {
        "non-fast-forward", "non fast forward", "merge conflict",
        "permission denied", "modulenotfounderror", "import error",
        "connection refused", "missing peer dependency",
    }
)
...
def score_specificity(trigger: str) -> Specificity:
    ...
    if len(tokens) == 2 and any(t in {"git", "docker", "npm", "pip", "kubectl", "pytest"} for t in tokens):
        return "moderate"
    return "generic"
