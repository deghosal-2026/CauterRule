...
def counterfactual(days: int) -> None:
    ...
    click.echo(f"Found {len(rules)} active rules and {len(failures)} historical failures (last {days}d).")
    click.echo("Counterfactual analysis:")
    for r in rules:
        prevented = sum(1 for t in failures if r.when.trigger.lower() in t.task.lower())
        if prevented:
            click.echo(f"  Rule {r.id} could have prevented {prevented} failure(s): \"{r.when.trigger}\"")